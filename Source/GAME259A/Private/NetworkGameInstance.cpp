...
UNetworkGameInstance::UNetworkGameInstance()
{
	...
	OnSessionUserInviteAcceptedDelegate = FOnSessionUserInviteAcceptedDelegate::CreateUObject(this, &UNetworkGameInstance::OnSessionUserInviteAccepted);

	/** Bind the function for completing the friend list request*/
	FriendListReadCompleteDelegate = FOnReadFriendsListComplete::CreateUObject(this, &UNetworkGameInstance::OnReadFriendsListCompleted);
...
bool UNetworkGameInstance::HostSession(TSharedPtr<const FUniqueNetId> UserId, FName SessionName, FString ServerName, bool bIsLAN, bool bIsPresence, int32 MaxNumPlayers, bool bIsPasswordProtected, FString SessionPassword)
{
...
void UNetworkGameInstance::OnSessionUserInviteAccepted(bool bWasSuccessful, int32 LocalUserNum, TSharedPtr<const FUniqueNetId> InvitingPlayer, const FOnlineSessionSearchResult& TheSessionInvitedTo)
{
	if (bWasSuccessful)
	{
...
void UNetworkGameInstance::SendSessionInviteToFriend(APlayerController* InvitingPlayer, const FBPUniqueNetId& Friend)
{
	IOnlineSubsystem* OnlineSub = IOnlineSubsystem::Get();
	if (OnlineSub)
	{
...
void UNetworkGameInstance::StartOnlineGame(FString ServerName, int32 MaxNumPlayers, bool bIsLAN, bool bIsPresence, bool bIsPasswordProtected, FString SessionPassword)
{
...
void UNetworkGameInstance::JoinOnlineGame(int32 SessionIndex)
{
	ULocalPlayer* const Player = GetFirstGamePlayer();
...
void UNetworkGameInstance::ShowErrorMessage(const FText& ErrorMessage)
{
	//Show the message on UMG through blueprints
	ShowErrorMessageUMG(ErrorMessage);
}

bool UNetworkGameInstance::IsOnlineSubsystemSteam() const
{
...
FString UNetworkGameInstance::GetPlayerName() const
{
...
IOnlineSessionPtr UNetworkGameInstance::GetSession() const
{
	IOnlineSubsystem* const OnlineSub = IOnlineSubsystem::Get();
...
UTexture2D* UNetworkGameInstance::GetSteamAvatar(const FBPUniqueNetId UniqueNetId)
{
...
void UNetworkGameInstance::GetSteamFriendsList(APlayerController* PlayerController)
{
...
void UNetworkGameInstance::OnReadFriendsListCompleted(int32 LocalUserNum, bool bWasSuccessful, const FString& ListName, const FString& ErrorString)
{
	if (bWasSuccessful)
	{
		//get the steam online subsystem
